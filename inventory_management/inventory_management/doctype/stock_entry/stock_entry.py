...
class StockEntry(Document):
        ...
        if self.type == "Consume" or self.type == "Transfer":
            for entry_item in self.entry_items:
                ...
                result = (
                    ...
                    .select(Sum(Ledger.qty_change))
                    .where(
                        (Ledger.warehouse == exiting_warehouse)
                        & (Ledger.item == item_name)
                ...
                if balance is None or balance < entry_item.quantity:
                    frappe.throw(
                        f"Not Enough Items to move out of warehouse: {self.from_warehouse}",
        ...
        for entry_item in self.entry_items:
            ...
            if self.type == "Receipt":
                self.create_ledger_entry(
                    entering_warehouse, item_name, qty_change, rate
                )
            elif self.type == "Consume":
                avg_value = self.get_current_avg_value(item_name, exiting_warehouse)
                self.create_ledger_entry(
                    exiting_warehouse, item_name, -qty_change, avg_value
                )
            else:
                avg_value = self.get_current_avg_value(item_name, exiting_warehouse)
                self.create_ledger_entry(
                ...
                self.create_ledger_entry(
                    entering_warehouse, item_name, qty_change, avg_value
                )

    def create_ledger_entry(self, warehouse, item_name, qty_change, rate):
        frappe.flags.allow_stock_ledger_insert = True
        ...
        frappe.flags.allow_stock_ledger_insert = False

    def get_current_avg_value(self, item_name, warehouse):
        Ledger = DocType("Stock Ledger")
        result = (
            frappe.qb.from_(Ledger)
            .select(Sum(Ledger.qty_change * Ledger.rate) / Sum(Ledger.qty_change))
            .where((Ledger.item == item_name) & (Ledger.warehouse == warehouse))
            .run()
        )
    ...
    def on_cancel(self):
        stock_ledger_items = frappe.db.get_all(
            "Stock Ledger", filters={"voucher_no": self.name}, fields=["*"]
        )
